...
EXPAND-ALIASES when non-nil expands note aliases for completion."
  (let* ((expanded-notes (if expand-aliases
                           ...
                           notes))
         ;; Build candidates as alist: (title . note)
         ;; Use vulpea-select-describe-fn for title only;
         ;; annotations handled by consult's :annotate
         (candidates
          (mapcar
           (lambda (note)
             (cons (funcall vulpea-select-describe-fn note) note))
           expanded-notes))
         (note (consult--read
                candidates
                ...
                :annotate (lambda (cand)
                            (when-let ((note (cdr (assoc cand candidates))))
